...
database_url = os.environ.get("POSTGRES_URI") or settings.POSTGRES_URI
if database_url:
    # Override the URL in the config
    config.set_main_option("sqlalchemy.url", database_url)
    print(f"Using database URL from environment/settings: {database_url}")
else:
    print("WARNING: No database URL found in environment or settings")
...
def run_migrations_online():
    ...
    print(f"Running migrations with URL: {config.get_main_option('sqlalchemy.url')}")
